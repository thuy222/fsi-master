...
class CognitoAdapter {
  /**
   *
   * Creates an instance of CognitoAdapter for an oidc-provider model.
   *
   * @constructor
  ...
  async upsert(id, payload, expiresIn) {
     ...
     */
    const indexOf = userData.findIndex((elem, index, arr) => {
      return elem.id === id
    })
    if (indexOf === -1) {
      userData.push({
   ...
   */
  async find(id) {
    const userIdx = userData.findIndex((elm, idx, arr) => {
      return elm.id === id
    })
  ...
  async consume(id) {
    ...
    userData[userIdx].payload.consumed = Math.floor(Date.now() / 1000)
  }
   ...
   */
  async destroy(id) {
    const userIdx = userData.findIndex((elm, idx, err) => {
      return elm.id === id
    })
